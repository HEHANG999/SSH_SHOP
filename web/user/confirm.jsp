...
<head>
    ...
    <% response.setHeader("cache-control", "no-store"); %><!-- 前台设置不缓存页面数据 -->
</head>
<body>
<c:if test="${empty sessionScope.order.itemEntities }">
<c:redirect url="/index.jsp"/><!-- 然后如果购物车中的购物项为空，则跳转到首页 -->
</c:if>
<div class="wrapper">
    <div class="header">
                ...
                <table class="data-table cart-table" cellpadding="0" cellspacing="0">
                    ...
                    <c:forEach items="${sessionScope.order.itemEntities}" var="item" varStatus="num">
                        <tr lang="${item.productEntity.id}">
                            ...
                            <td width="80px"><img src="${shop}/upload/${item.productEntity.pic}" width="80"
                                                  height="80" />
                            </td>
                            <td class="align_left"><a class="pr_name" href="#">${item.name}</a>
                            </td>
                            <td class="align_center vline">￥ ${item.price}</td>
                            <td class="align_center vline">
                                    ${item.number}
                            </td>
                            <td class="align_center vline">￥${item.price*item.number}</td>
                        </tr>
                    </c:forEach>
                </table>

                <div class="sum">
                    <div class="fr"><span>总计：</span><b>￥${order.total}</b></div>
                </div>
            ...
            <form action="${shop}/order_save.action" method="post">
                <div class="person-check check">
                    ...
                    <div class="person-checkinner">
                        ...
                        <div>
                            <label>区域邮编:</label>
                            <input type="text" name="post" value="${sessionScope.user.email }"/>
                        </div>
                        <div>
                            <label>配送地址:</label>
