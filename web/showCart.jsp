		...
		$(function(){
			...
			//验证数据的有效性
				...
				if(!isNaN(number) && parseInt(number)==number && number>0){ 
					...
					$.post("item_updateItem.action", {number:number, 'productEntity.id':pid}, function(total){
				...
				<div class="top_bar clear">
					...
					<div class="minicart">
						<a class="minicart_link" href="#"> <span class="item">
								<b>${fn:length(sessionScope.order.itemEntities)}</b> 件/ </span>
								<span class="price"> <b>￥${sessionScope.order.total}</b> </span> </a>
					</div>
					<!-- 小购物车结束 -->
					<!-- 搜索框 -->
		...
		<div class="section_container">
			...
			<div id="shopping_cart">
				...
				<table class="data-table cart-table" cellpadding="0" cellspacing="0">
					...
					<c:forEach items="${sessionScope.order.itemEntities }" var="item" varStatus="num">
					<tr lang="${item.productEntity.id}">
						<td class="align_center"><a href="#" class="edit">${num.count }</a>
						</td>
						<td width="80px"><img src="${shop}/upload/${item.productEntity.pic}" width="80" height="80" />
						</td>
						<td class="align_left"><a class="pr_name" href="#">${item.name }</a>
						</td>
						...
						<td class="align_center vline">
							<!-- 文本框 -->
							<input class="text" style="height: 20px;" value="${item.number }" lang="${item.number }">
						</td>
						<td class="align_center vline">
						...
						<tbody>
							<tr>
								...
								<td class="align_right" style=""><strong>￥<span
										class="price" id="total">${sessionScope.order.total}</span>
								</strong>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="align_right" style="">￥<span class="total" id="totalAll"><strong>${sessionScope.order.total}</strong>
								</span>
								</td>
							</tr>
						</tbody>
